...
//Continuar para a próxima iteração (continue to the next iteration)
    //O comando 'continue' é uma "versão mais leve" do 'break'. Não interrompe todo o loop, interrompe a iteração atual e força o loop a começar uma nova (se a condição permitir).
    ...
    //Para valores pares de 'i', o comando 'continue' para a execução do corpo e passa controle para a próxima iteração de 'for' (com o próximo número). Então o console.log só chamado para valores ímpares.

            //O comando 'continue' ajuda a diminuir aninhamento
            ...
            //Sem break/continue ao lado direito de '?'
                //Por favor note que construções de sintaxe as quais não são expressões não podem ser usadas com o operador ternário '?'. Em particular, comandos como break/continue não são permitidas nele.
